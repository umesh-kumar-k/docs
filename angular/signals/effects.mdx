...
```
❌ Propagation: effect(() => name.set(engName())); // Cycles!
❌ Imperative: Less declarative/reactive
❌ Events: Signals glitch-free (batch rapid changes)
```
- Glitch-free: Multiple rapid sets → effect sees only final value
...
3. RxJS: toObservable → operators → toSignal
...
- **Declarative Chain**: Events → RxJS → Signals (unidirectional)
...
```
✅ Rendering: Logging/canvas/SnackBar imperative API
✅ computed(): Synchronous derivation
✅ RxJS: Async/race-safe (debounce/switchMap)
✅ rxMethod: Store delegation
✅ Events: Behind signal changes (not signal tracking)
❌ Propagation: effect writes → cycles/glitches
❌ Business Logic: Use computed/events/RxJS
❌ Auto-tracking Pitfalls: explicitEffect last resort
```
...
| Mutable events | Glitch-free ignored | Events → RxJS |

## Key Highlights & Tradeoffs

```
🏆 Effects Summary:
✅ Rendering: Logging/canvas/UI imperative
❌ Propagation: computed()/RxJS/events
❌ Async: rxMethod/Resource API

⚖️ effect vs computed:
| effect | computed |
|--------|----------|
...
Angular Effects are for side effects (logging, imperative UI calls, external APIs), not for synchronizing or deriving state; Angular 19’s `linkedSignal` gives a writable, source‑driven signal that cleanly keeps state in sync without effect‑based hacks or glitches.[1][2]
...
- Issues:
  - Effects are asynchronous and can introduce transient inconsistent states (“glitches”).  
  - They blur concerns by mixing state derivation with side effects, making code harder to reason about.  
  - Recommended scope for `effect`: non‑renderable side effects like logging, SnackBar, canvas, etc.[4][3]

### Example: ProductCard anti‑pattern
...
- “Fix” with effect:
  ...
  ```
  which is exactly the anti‑pattern: using effect for state sync.[2][1]
...
- Behavior:
  ...
  - `amount.update(v => v + 1)` still works; it’s writable local state tied to a source.[5][1]
- Benefits:
  - Immediate, glitch‑free updates (no async effect scheduling).  
  - No effect needed; clearer separation of concerns.  
  - Purpose‑built API for “state that tracks another signal but stays writable”.[6][5]

## Important APIs & concepts

- `effect(fn)`: runs whenever one or more signals it *reads* change; auto‑tracks dependencies, can cause unintended coupling if used for state propagation.[4][3]
...
- Use **Effects** for:
  ...
  - Bridging reactive world to non‑reactive services.[3][4]
...
- Avoid `allowSignalWrites` in effects except for framework‑level or very rare cases; prefer `linkedSignal` or stores instead.[8][3]

## Advanced notes (senior‑level)
...
- Architecturally: effects → edges of system; signals/linkedSignal/computed → core domain state.

## Interview cheat sheet (Q&A)

- **Q: Why is using `effect` to sync signals considered an anti‑pattern?**  
  A: Because effects are for side effects; using them for state sync mixes concerns, depends on auto‑tracking, and can introduce glitches due to async execution.[1][3]

- **Q: What does `linkedSignal` give you that `computed` does not?**  
  A: `linkedSignal` is writable and still recomputes when the source changes; `computed` is read‑only and purely derived.[5][7]

- **Q: When would you pick linkedSignal over an effect?**  
  A: Whenever you want local state that should reset/track a source signal (like amount depending on product) but remains user‑editable.[2][1]

- **Q: What are good uses of effects in a signals‑first app?**  
  A: Logging, triggering SnackBars, talking to browser APIs, or other non‑reactive systems—never for primary state propagation.[4][3]

## Key terms & keywords (scan list)

- effect misuse / anti‑pattern  
- state synchronization vs side effects  
- `linkedSignal({ source, computation })`  
- writable + reactive signal  
- immediate, glitch‑free updates  
