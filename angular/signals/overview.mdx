...
```
flightRoute = computed(() => `${this.from()} → ${this.to()}`);  // Derived, memoized
...
**Injection Context**
```
constructor() { effect(() => console.log(this.from())); }  // ✅
ngOnInit() { effect(() => ...); }                          // ❌ NG0203
runInInjectionContext(inject(Injector), () => effect(...)) // ✅ Manual
```
...
```
setTimeout(() => {
  this.from.set('London');  // No "London → Graz" interim state
  this.to.set('Paris');
}, 2000);
```
...
```
✅ Immutables: {...old, prop: newVal} not mutation
✅ OnPush: Signals = natural trigger
✅ computed(): flightCount = computed(() => flights().length)
✅ effect(): Logging/toast/analytics only
✅ RxJS Interop: debounceTime → toSignal for typeahead
❌ Mutation: flights()[0].date = newDate // Invisible
❌ effect(): Business logic (use computed/signal)
```
...
```
Glitch Prevention:
from.set('A'); to.set('B'); // No "A→Graz" interim computed()
...
**Q: RxJS → Signal race condition?**  
A: `toSignal(obs$, {initialValue: []})`; `requireSync: true` for BehaviorSubject 

## Key Highlights & Tradeoffs

```
🏆 Highlights: Glitch-free, granular reactivity, RxJS interop, OnPush native
⚖️ Tradeoffs:
| Signal | Observable |
|--------|------------|
