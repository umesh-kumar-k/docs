...
- RxJS Subject→Observable pattern equivalent
...
- **Readonly Projection**: Private writable → public readonly signals
...
```
✅ Immutables: {...old, prop: new} not mutation
✅ OnPush: Signal bindings trigger checks
✅ Facade Services: State persistence, sharing
✅ computed(): Derived state (flights.length → count)
✅ patchSignal(): Partial updates (Component Store style)
❌ Mutation: array.push() breaks reactivity
```
...
- **Angular Team**: Signals → future granular CD (no ancestor pollution)
...
```
🏆 Highlights: Fine-grained reactivity, OnPush perf, service sharing
⚖️ Tradeoffs:
