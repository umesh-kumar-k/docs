...
```
- No OnPush, hidden deps (ratings→desserts), maintenance nightmare
...
```
✅ effect(() => toastService.show(desserts().length + ' loaded')); // Logging/UI
❌ effect(() => originalName.set(englishName())); // State propagation (prohibited)
...
- State propagation → untracked() + events/Observables
...
Store: Actions → Effects → Reducers → Signals → computed slices
```
```
...
```
- Unidirectional: Events → Store → Signals (no component state)
...
- **toObservable(signal)**: Signal → Observable (RxJS interop)
- **NGRX Signal Store**: Actions → Effects → Signal state slices

## Design Patterns

- **Reactive Derivation**: `computed()` eliminates manual sync
- **Unidirectional Flow**: Events → Store → Derived Signals
- **Effect Boundaries**: Side-effects only (no business logic)

## Best Practices

```
✅ computed(): Always derive (ratedDesserts = toRated(desserts(), ratings()))
✅ Observables: Async + race-safe (switchMap/debounceTime)
✅ Stores: Offload complexity (NGRX Signal Store)
✅ OnPush: Default with signals
✅ untracked(): Break reactive context in effects
❌ Effects writes: Prohibited (use events/Observables)
❌ Manual sync: desserts.map() after ratings change
```
...
store.updateFilter(filter); // Action → derived signals
```
...
**Q: Async signals?**  
A: Observables → toSignal() or stores (Resource API future) 
...
```
🏆 Rules Summary:
1️⃣ Derive: computed() > manual sync
2️⃣ Effects: Side-effects only (no writes)
3️⃣ Stores: Async + unidirectional flow
