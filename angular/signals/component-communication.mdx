...
- **Aliases**: `{alias: 'title'}` → `<app-opt title="foo">`
...
```
- **Handles**: `#tab` → `contentChildren(TabCmp, {selector: '#tab'})`
...
- **model<T>()**: → ModelSignal<T> (2-way)
- **output<T>()**: → OutputEmitterRef<T>
- **contentChildren<T>()/viewChildren<T>()**: → Signal<T[]>
- **contentChild<T>()/viewChild<T>()**: → Signal<T|undefined>
...
```
✅ computed/effect: Replace ngOnChanges/ngOnInit for inputs
✅ input.required: Mandatory props (no ctor access)
✅ {descendants: true}: Nested queries when needed
✅ outputFromObservable: Dynamic component RxJS events
✅ OnPush: Native signal reactivity
❌ Constructor: input.required() access (undefined)
❌ Aliases: Unless directive selector mismatch
```
...
| No 2-way binding | Missing model() | `[(prop)]="value"` → model() |
...
```
🏆 Highlights: Reactive queries, native 2-way, granular updates
⚖️ Tradeoffs:
| New API | Legacy |
|---------|---------|
| Signals | QueryList |
